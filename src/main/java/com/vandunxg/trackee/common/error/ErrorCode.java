package com.vandunxg.trackee.common.error;

import lombok.Getter;

import com.vandunxg.trackee.common.constant.MessageKey;

@Getter
public enum ErrorCode {

    // COMMON ERRORS
    VALIDATION_ERROR(MessageKey.VALIDATION_ERROR),
    INTERNAL_SERVER_ERROR(MessageKey.INTERNAL_SERVER_ERROR),
    REQUIRED_REQUEST_BODY(MessageKey.REQUIRED_REQUEST_BODY),

    // AUTH
    AUTH_INVALID_CREDENTIALS(MessageKey.AUTH_INVALID_CREDENTIALS),
    AUTH_TOKEN_INVALID(MessageKey.AUTH_TOKEN_INVALID),
    AUTH_VERIFICATION_TOKEN_INVALID(MessageKey.AUTH_VERIFICATION_TOKEN_INVALID),
    AUTH_VERIFICATION_TOKEN_EXPIRED(MessageKey.AUTH_VERIFICATION_TOKEN_EXPIRED),
    AUTH_VERIFICATION_TOKEN_NOT_FOUND(MessageKey.AUTH_VERIFICATION_TOKEN_NOT_FOUND),
    AUTH_VERIFICATION_TOKEN_USED(MessageKey.AUTH_VERIFICATION_TOKEN_USED),
    AUTH_VERIFICATION_TOKEN_FORMAT_INVALID(MessageKey.AUTH_VERIFICATION_TOKEN_FORMAT_INVALID),
    AUTH_VERIFICATION_TOKEN_SIGNATURE_INVALID(MessageKey.AUTH_VERIFICATION_TOKEN_SIGNATURE_INVALID),
    AUTH_VERIFICATION_TOKEN_USER_MISMATCH(MessageKey.AUTH_VERIFICATION_TOKEN_USER_MISMATCH),
    AUTH_VERIFICATION_TOKEN_ALREADY_VERIFIED(MessageKey.AUTH_VERIFICATION_TOKEN_ALREADY_VERIFIED),
    AUTH_VERIFICATION_TOKEN_BLOCKED(MessageKey.AUTH_VERIFICATION_TOKEN_BLOCKED),

    // USER ERRORS
    USER_NOT_FOUND(MessageKey.USER_NOT_FOUND),
    USER_EMAIL_EXISTS(MessageKey.USER_EMAIL_EXISTS),
    USER_CREATED(MessageKey.USER_CREATED),
    USER_UPDATED(MessageKey.USER_UPDATED),
    USER_DELETED(MessageKey.USER_DELETED),
    USER_INACTIVE(MessageKey.USER_INACTIVE),

    // USER VALIDATION
    EMAIL_BLANK(MessageKey.EMAIL_BLANK),
    EMAIL_INVALID(MessageKey.EMAIL_INVALID),

    PASSWORD_BLANK(MessageKey.PASSWORD_BLANK),
    PASSWORD_INVALID(MessageKey.PASSWORD_INVALID),
    PASSWORD_MISMATCH(MessageKey.PASSWORD_MISMATCH),
    PASSWORD_TOO_SHORT(MessageKey.PASSWORD_TOO_SHORT),
    PASSWORD_TOO_LONG(MessageKey.PASSWORD_TOO_LONG),
    PASSWORD_TOO_WEAK(MessageKey.PASSWORD_TOO_WEAK),
    PASSWORD_NO_DIGIT(MessageKey.PASSWORD_NO_DIGIT),
    PASSWORD_NO_UPPERCASE(MessageKey.PASSWORD_NO_UPPERCASE),
    PASSWORD_NO_LOWERCASE(MessageKey.PASSWORD_NO_LOWERCASE),
    PASSWORD_NO_SPECIAL_CHAR(MessageKey.PASSWORD_NO_SPECIAL_CHAR),
    ;

    private final String messageKey;

    ErrorCode(String messageKey) {
        this.messageKey = messageKey;
    }
}
